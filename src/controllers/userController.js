...
const infoUserBank = async (req, res) => {
  let auth = req.cookies.auth;
  let timeNow = Date.now(); // define it early so it’s always accessible
  ...
  try {
    ...
    const [lastRecharge] = await connection.query(
      "SELECT * FROM recharge WHERE phone = ? AND status = 1 ORDER BY time DESC LIMIT 1",
      [userInfo.phone]
    );
    const lastRechargeRemainingBet =
    ...
    const [lastBetResetTime] = await connection.query(
      "SELECT time FROM recharge WHERE phone = ? AND status = 1 AND remaining_bet = 0 ORDER BY time DESC LIMIT 1",
      [userInfo.phone]
    );

    let betTimeInterval = "";
    if (lastBetResetTime.length > 0) {
      // ✅ Wrap time value in quotes to avoid SQL syntax error
      const lastTime = lastBetResetTime[0].time;
      betTimeInterval = `AND time > '${lastTime}'`;
    }

    // ***************************************** GIFT CARD USED *****************************************
    const [giftCardUsed] = await connection.query(
      `SELECT SUM(money) AS giftCardTotalAmount FROM redenvelopes_used WHERE phone_used = ? ${betTimeInterval}`,
      [userInfo.phone]
    );
    ...
    const [claimRewards] = await connection.query(
      ...
      ${betTimeInterval}
      `,
      [userInfo.phone, ...specificRewardTypes]
    );
    ...
    const [wingoLossResult] = await connection.query(
      `SELECT SUM(money) AS totalMoney, SUM(fee) AS totalFees FROM minutes_1 WHERE phone = ? ${betTimeInterval}`,
      [userInfo.phone]
    );
    const wingoLossMoney =
      (parseFloat(wingoLossResult[0].totalMoney) || 0) +
      (parseFloat(wingoLossResult[0].totalFees) || 0);

    const [k3LossResult] = await connection.query(
      `SELECT SUM(money) AS k3LossMoney FROM result_k3 WHERE phone = ? ${betTimeInterval}`,
      [userInfo.phone]
    );
    const k3LossMoney = Number(k3LossResult[0].k3LossMoney) || 0;

    const [G5LossResult] = await connection.query(
      `SELECT SUM(money) AS G5dLossMoney FROM result_5d WHERE phone = ? ${betTimeInterval}`,
      [userInfo.phone]
    );
    const G5dLossMoney = Number(G5LossResult[0].G5dLossMoney) || 0;

    const [trxWingoLossResult] = await connection.query(
      `SELECT SUM(money) AS trxWingoLossMoney FROM trx_wingo_bets WHERE phone = ? ${betTimeInterval}`,
      [userInfo.phone]
    );
    ...
    const [withdrawResult] = await connection.query(
      `SELECT SUM(money) AS totalWithdraw FROM withdraw WHERE phone = ? AND status = 1`,
      [userInfo.phone]
    );
    const totalWithdraw = Number(withdrawResult[0].totalWithdraw) || 0;

    const [withdrawPendingResult] = await connection.query(
      `SELECT SUM(money) AS totalWithdrawPending FROM withdraw WHERE phone = ? AND status = 0`,
      [userInfo.phone]
    );
    ...
    const allowedWithdrawAmount = true;
    ...
    return res.status(200).json({
      ...
      userInfo: {
        ...
        phone_user: userInfo.phone,
        money_user: userInfo.money,
        bonus_money: userInfo.bonus_money,
