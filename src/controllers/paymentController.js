...
const PaymentMethodsMap = {
  ...
  USDT: "usdt",
  UPAY: "upay",
  CLOUD_PAY: "cloud_pay",   // ✅ Add this line
};
...
function cloudpaySign({ amount, currency, merch_id, order_id, redirect_url, callback_url, remark }, secret) {
  const canonical =
    `amount=${amount}|callback_url=${callback_url}|currency=${currency}|merch_id=${merch_id}|order_id=${order_id}|redirect_url=${redirect_url}|remark=${remark ?? ""}`;
  return crypto.createHmac("sha256", secret).update(canonical).digest("hex");
}

// POST /wallet/pay/cloudpay?am=123.45
const initiateCloudpayPayment = async (req, res) => {
  const type = PaymentMethodsMap.CLOUD_PAY;
  const auth = req.cookies.auth;
  const amount = parseFloat(req.query.am);
  ...
  try {
    ...
    const merchId = process.env.CLOUDPAY_MERCH_ID;
    const secret = process.env.CLOUDPAY_SECRET;

    if (!merchId || !secret) {
      console.error("CloudPay env vars missing");
      return res.status(500).send("Payment configuration error.");
    }

    const orderId = getRechargeOrderId();
    const payload = {
      amount: amount.toFixed(2),         // as string with 2 decimals
      currency: "INR",
      merch_id: merchId,
      order_id: orderId,
      redirect_url: `${process.env.APP_BASE_URL}/wallet/rechargerecord`,
      callback_url: `${process.env.APP_BASE_URL}/wallet/verify/cloudpay`,
      remark: String(phone || ""),       // free-form; we store user phone
    };

    const signature = cloudpaySign(payload, secret);

    const response = await axios({
      method: "POST",
      url: "https://api.cloudpay.space/api/v1/order",
      data: payload,
      headers: {
        "content-type": "application/json",
        "X-Verify": signature,
    ...
    // CloudPay response shape: assume { code: 200, data: { pay_url, order_id, amount } } (be defensive)
// CloudPay response handling (supports both old/new shapes)
...
const ok =
  ...
  resp.success === true ||
  resp.status === true ||
  /success/i.test(resp.message || "");

// Some providers use `data`, yours uses `result`
const data = resp.data || resp.result || {};

// Normalize fields
const payUrl = data.pay_url || data.payment_url || data.payUrl || data.paymentUrl;
const providerOrderId =
  data.order_id || data.orderId || resp.order_id || resp.orderId || payload.order_id;
// CloudPay-specific transaction reference (if present)
const cldno = data.cldno;

// If success & we have a payment URL + order id, proceed
if (ok && payUrl && providerOrderId) {
  const newRechargeParams = {
    orderId: payload.order_id,
    transactionId: cldno || providerOrderId, // prefer gateway's unique id if provided
    utr: 0,
    phone,
    money: data.amount ?? payload.amount,     // their response didn’t include amount; fall back
    type,
    status: PaymentStatusMap.PENDING,
    today: rechargeTable.getCurrentTimeForTodayField(),
    url: payUrl,
...
// Not successful — log full body for debugging
console.log("CloudPay create-order response:", response.data);
return res.status(400).send("Something went wrong! Please try again later.");

  } catch (error) {
    console.log("CloudPay initiate error:", error?.response?.data || error);
    return res.status(500).json({
      status: false,
      message: "Something went wrong!",
      timestamp: timeNow,
...
// Configure URL:  POST  ${APP_BASE_URL}/wallet/verify/cloudpay
const verifyCloudpayPayment = async (req, res) => {
  try {
    // Typical CloudPay callback fields (defensive parsing)
    // Expecting JSON and optional header 'x-verify' for signature (lowercase by Node)
    const data = req.body || {};
    const headerSig = req.get("X-Verify") || req.get("x-verify") || "";

    const merch_id = data.merch_id;
    const order_id = data.order_id;            // our order id
    const transaction_id = data.transaction_id; // CloudPay internal id (if provided)
    const amount = data.amount;
    const currency = data.currency || "INR";
    const status = (data.status || "").toString().toUpperCase(); // e.g. PAID | PENDING | FAILED
    const remark = data.remark ?? "";
    const callback_url = `${process.env.APP_BASE_URL}/wallet/verify/cloudpay`;
    const redirect_url = `${process.env.APP_BASE_URL}/wallet/rechargerecord`;

    // Basic validation
    if (!merch_id || !order_id || !status || !amount) {
      return res.status(400).send("Invalid Request!");
    }

    if (merch_id !== process.env.CLOUDPAY_MERCH_ID) {
      return res.status(401).send("failed");
    }

    // If CloudPay signs callbacks the same way, verify (safe to attempt; if header absent, we log and continue like your RS Pay flow)
    try {
      const expectedSig = cloudpaySign(
        {
          amount: Number(amount).toFixed(2),
          currency,
          merch_id,
          order_id,
          redirect_url,   // must match what we used when creating the order
          callback_url,   // must match too
          remark: String(remark ?? ""),
        },
        process.env.CLOUDPAY_SECRET
      );

      if (headerSig && headerSig !== expectedSig) {
        console.warn("CloudPay callback signature mismatch", { headerSig, expectedSig });
        // You can choose to reject here; mirroring your RS Pay flow, we'll proceed but log it.
      }
    } catch (e) {
      console.warn("CloudPay callback signature check error:", e?.message);
    }

    const recharge = await rechargeTable.getRechargeByOrderId({ orderId: order_id });
    if (!recharge) {
      console.log({
        message: `CloudPay verification: recharge not found`,
        order_id,
        timeStamp: timeNow,
      });
      return res.status(400).send("failed");
    ...
    // Map statuses to your internal logic
    const CLOUDPAY_STATUS = {
      SUCCESS: ["PAID", "SUCCESS", "SUCCESSFUL"],
      PROCESSING: ["PENDING", "PROCESSING"],
      FAILED: ["FAILED", "CANCELED", "CANCELLED", "REJECTED"],
      PARTIAL: ["PARTIAL", "PARTIALLY_PAID"],
    };

    const inSet = (set, v) => set.includes(v);

    if (inSet(CLOUDPAY_STATUS.SUCCESS, status)) {
      const user = await getUserDataByPhoneNumber(recharge.phone);

      await rechargeTable.setStatusToSuccessByIdAndOrderId({
        id: recharge.id,
        orderId: recharge.orderId,
        utr: transaction_id || data.utr || 0,
      });
      ...
      return res.status(200).send("success");
    } else if (inSet(CLOUDPAY_STATUS.PROCESSING, status)) {
      return res.status(200).send("processing");
    } else if (inSet(CLOUDPAY_STATUS.FAILED, status)) {
      await rechargeTable.cancelById(recharge.id);
      return res.status(200).send("failed");
    } else if (inSet(CLOUDPAY_STATUS.PARTIAL, status)) {
      // Adjust as you prefer; here we cancel & surface partial
      await rechargeTable.cancelById(recharge.id);
      return res.status(200).send("partially success");
    }
    ...
    console.log("CloudPay verify error:", error);
    return res.status(500).json({
      message: "Something went wrong!",
      errorMessage: error.message,
      error: error,
