...
//   let sql =
//     "SELECT id, id_user, phone, level, money, status, plain_password, today FROM users WHERE veri = 1 AND level != 2";
//   let countSql =
//     "SELECT COUNT(*) as total FROM users WHERE veri = 1 AND level != 2";
//   let params = [];
...
const rechargeonly = async (req, res) => {
  try {
    const auth = req.cookies.auth;
    ...
    // Status Filter
    const filterStatus = req.query.status; // 1 = success, 2 = failed, 0 = pending
    ...
    const [totalCountResult] = await connection.query(
      `SELECT COUNT(*) AS total FROM recharge ${whereQuery}`,
      whereParams,
    );
    ...
    const [rechargeRecords] = await connection.query(
      `SELECT * FROM recharge ${whereQuery} ORDER BY id DESC LIMIT ? OFFSET ?`,
      [...whereParams, limit, offset],
    );

    // Add readable status text
    const finalData = rechargeRecords.map((data) => {
      return {
        ...data,
        statusText:
          data.status == 1
    ...
    console.error("Error in rechargeonly:", error);
    const timeNow = new Date().toISOString();
    res.status(500).json({
...
const deleteuser = async (req, res) => {
  try {
    ...
    // 1️⃣ Fetch user from main table
    ...
    // 2️⃣ Insert into deleteuser_table
    await connection.query(
      `INSERT INTO deleteuser_table (
        original_user_id,
        id_user,
        dial_code,
        phone,
    ...
    // 3️⃣ Now delete user from main table
...
export const updateAdminFields = async (req, res) => {
  const { field, value, phone } = req.body;
  ...
  try {
    if (!phone) {
      return res
        .status(400)
        .json({ success: false, message: "Phone Number is requried" });
    }
    ...
    // 4️⃣ Skip update if no valid field provided
    if (!field || !allowedFields[field]) {
      return res.json({
        success: true,
        message: "No changes made (no valid field provided)",
      });
    }

    const fieldToUpdate = allowedFields[field];

    // 5️⃣ Update logic (optional)
    if (fieldToUpdate === "plain_password" && value && value.length < 6) {
      return res.status(400).json({
        success: false,
    ...
    if (
      fieldToUpdate === "account_number" &&
      value &&
      !/^\d{10,}$/.test(value)
    ) {
      return res.status(400).json({
        success: false,
        message: "Account number must contain at least 10 digits",
      });
    }

    if (fieldToUpdate === "plain_password" && value) {
      const hashedPassword = await bcrypt.hash(value, saltRounds);
      await connection.execute(
        "UPDATE users SET password = ?, plain_password = ? WHERE phone = ?",
        [hashedPassword, value, phone],
      );
    } else if (fieldToUpdate === "avatar" && req.file) {
      const avatarFile = req.file.filename;

      // Delete old avatar if exists
      const oldAvatar = rows[0].avatar;

      const oldAvatarPath = oldAvatar ? path.join(__dirname, oldAvatar) : null;

      if (oldAvatar && fs.existsSync(oldAvatarPath)) {
        fs.unlinkSync(oldAvatarPath);
      }

      await connection.execute("UPDATE users SET avatar = ? WHERE phone = ?", [
        avatarFile,
        phone,
      ]);
      return res.json({
    ...
    } else if (fieldToUpdate === "extra_time" && value) {
      const dateValue = new Date(value);
      if (!isNaN(dateValue.getTime())) {
        await connection.execute(
          "UPDATE users SET extra_time = ? WHERE phone = ?",
          [dateValue, phone],
        );
      }
    } else if (fieldToUpdate === "recipient_name" && value) {
      await connection.execute(
        "UPDATE user_bank SET name_user = ? WHERE phone = ?",
        [value, phone],
      );
    } else if (fieldToUpdate === "money" && value) {
      await connection.execute("UPDATE users SET money = ? WHERE phone = ?", [
        value,
        phone,
      ]);
    } else if (fieldToUpdate === "name_user" && value) {
      await connection.execute(
        "UPDATE users SET name_user = ? WHERE phone = ?",
        [value, phone],
      );
    } else if (fieldToUpdate === "total_refers" && value) {
      await connection.execute(
        "UPDATE users SET total_refers = ? WHERE phone = ?",
        [value, phone],
      );
    } else if (fieldToUpdate === "total_inviters" && value) {
      await connection.execute(
        "UPDATE users SET total_inviters = ? WHERE phone = ?",
        [value, phone],
      );
    } else if (value) {
      await connection.execute(
        `UPDATE user_bank SET ${fieldToUpdate} = ? WHERE phone = ?`,
        [value, phone],
      );
    }
    ...
    return res.json({
      success: true,
      message:
        fieldToUpdate === "avatar" && req.file
          ? "avatar updated successfully"
          : value
            ? `${fieldToUpdate.replace("_", " ")} updated successfully`
            : "No changes made (no value provided)",
    });
  } catch (error) {
...
export const dashboardData = async (req, res) => {
  ...
  const [todayUsers] = await connection.query(
    `SELECT COUNT(id) AS total FROM users WHERE DATE(today) = CURDATE()`,
  );
  ...
  const [todayRecharge] = await connection.query(
   ...
   FROM recharge 
   WHERE status = 1 
   AND DATE(STR_TO_DATE(today, '%Y-%d-%m %r')) = CURDATE()`,
  );
  ...
  const [todayWithdraw] = await connection.query(
    `SELECT SUM(money) as total FROM withdraw WHERE status = 1  AND DATE(today) = CURDATE()`,
  );

  const [win] = await connection.query(
    `SELECT SUM(money) as total FROM minutes_1 WHERE status = 1 `,
  );
  const [todayWin] = await connection.query(
    `SELECT SUM(money) as total FROM minutes_1 WHERE status = 1 AND DATE(today) = CURDATE()`,
  );
  const [loss] = await connection.query(
    `SELECT SUM(money) as total FROM minutes_1 WHERE status = 2 `,
  );
  const [todayLoss] = await connection.query(
    `SELECT SUM(money) as total FROM minutes_1 WHERE status = 2 AND DATE(today) = CURDATE()`,
  );
  ...
  return res.status(200).json({
    ...
    blockedUsers: blockedUsers[0].total ?? 0,
    totalUsers: totalUsers[0].total ?? 0,
    todayUsers: todayUsers[0].total ?? 0,
    recharges: recharge[0].total ?? 0,
    todayRecharge: todayRecharge[0].total ?? 0,
    withdraws: withdraw[0].total ?? 0,
    todayWithdraws: todayWithdraw[0].total ?? 0,
    todayWin: todayWin[0].total ?? 0,
    todayLoss: todayLoss[0].total ?? 0,
  });
};
