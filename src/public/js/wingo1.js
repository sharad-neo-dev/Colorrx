...
socket.on("data-server", function (msg) {
  ...
  $(".Loading").fadeIn(0);
  setTimeout(() => {
    let data1 = msg.data[0]; // Get the newest bridge
    let data2 = []; // Take out the old bridge
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        for(let i=0;i<5;i++){
        let ele = data[i];
        $("#result_container").append(`<img src="/wingo/n${ele.amount}.png" />`);
    }
...
function openAudio() {
  audio1.muted = true;
  audio2.muted = true;
//   $(".item-volume").attr('src','/img/icon/voice-62dbf38c.png');
  if(sound){
//   $(".item-volume").attr('src','/img/icon/voice-off-633f5ccc.png');
    audio1.play();
    audio2.play();
  }
}
...
fetch("/api/webapi/GetUserInfo")
  ...
  .then((data) => {
    ...
    $(".num span").text(`₹ ${data.data.money_user} `);
    $("#numberMobileUser").val(data.data.phone_user);
  });
...
function showListOrder2(list_orders, x) {
  ...
  let result = list_orders.map((list_orders) => {
    ...
    if ((join == 'd' && res == 0) || (join == 'x' && res == 5)) {
        valueation = 1.5;
    }
    if ((!isNumber(join) && join == "l") || join == "n") {
      checkJoin = `
                    <div data-v-a9660e98="" class="van-image" style="width: 40px !important; height: 40px!important;">
                        <img src="/images/${
                          join == "n" ? "Small" : "Big"
                        }.png" class="van-image__img"  style="width: 40px !important; height: 40px!important;">
                    </div>
                    `;
    } else {
      checkJoin = `
                            ...
                            <div data-v-a9660e98="" class="li c-row c-row-between c-row-middle">
                                ...
                                <div data-v-a9660e98="" class="li c-row c-row-between c-row-middle">
                                    ...
                                    <div data-v-a9660e98="">
                                        ...
                                        <div data-v-a9660e98="" style="display: inline-block; margin-left: 8px;">${
                                          list_orders.result < 5 ? "Small" : "Big"
                                        }</div>
                                    </div>
                                </div>
                                <div data-v-a9660e98="" class="li c-row c-row-between c-row-middle ">
...
$(`.game-betting .box .item .img .van-image img`).attr('src', '/img/icon/time-5d4e96a3.png');
$(`.game-betting .box .item:eq(${selectPageTime - 1}) .img .van-image img`).attr('src', '/img/icon/time_a-f83ed4c7.png');