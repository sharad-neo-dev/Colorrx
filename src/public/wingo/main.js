...
function countDownTimer({ GAME_TYPE_ID }) {
  ...
  const getTimeMSS = (countDownDate) => {
    ...
    // 🔥 30 SECOND GAME
    if (parseInt(GAME_TYPE_ID) === 30) {
      seconds = seconds % 30;
    } else {
      // normal minute based game
      const minutes = Math.floor((distance % (1000 * 60 * 60)) / (1000 * 60));
      minute = Math.ceil(minutes % parseInt(GAME_TYPE_ID));
    }
  ...
  countDownInterval2 = setInterval(() => {
    ...
    if (minute == 0 && seconds1 == 2 && seconds2 == 5) {
      if (clicked && check_volume === "on") playAudio2();
    }
  }, 1000);

  // ⛔ Betting lock
  countDownInterval3 = setInterval(() => {
    const { minute, seconds1, seconds2 } = getTimeMSS(countDownDate);

    if (minute == 0 && seconds1 == 0 && seconds2 <= 5) {
      $(".van-overlay, .popup-join").fadeOut();
      $(".Betting__C-mark").css("display", "flex");
      $(".Betting__C-mark div:eq(0)").text(seconds1);
      $(".Betting__C-mark div:eq(1)").text(seconds2);
    } else {
      $(".Betting__C-mark").hide();
    }
  }, 1000);
}
...
function updateGameTitle(GAME_TYPE_ID) {
  let title = "Win Go";

  if (GAME_TYPE_ID === 30) {
    title += " 30Sec";
  } else {
    title += ` ${GAME_TYPE_ID}Min`;
  }

  document.querySelector(".TimeLeft__C-name").innerText = title;
}


const selectActiveClockByGameType = (GAME_TYPE_ID) => {
    
     updateGameTitle(GAME_TYPE_ID);
...
function selectActiveClock(currentTime) {
  ...
  document.querySelector(".min_t_30").classList.remove("active");
  ...
  switch (parseInt(currentTime)) {
    ...
    case 5:
      ...
      break;
    case 30:
      document.querySelector(".min_t_30").classList.add("active");
      break;
    default:
...
const displayResultHandler = ({ status, amount, period, result,GAME_TYPE_ID }) => {
  let colorDisplay = "";
  let bsDisplay = "";
  ...
  if(GAME_TYPE_ID == 30){
       $("#popup_game_details").html(`Period: Win ${GAME_TYPE_ID} secound ${period}`); 
  }else{
  ...
  try {
    ...
    const lastGameBets = betListData?.filter(
      (game) => game.stage === lastGame?.period,
        ...
        const lostGames = lastGameBets?.filter((game) => game.status !== 1);
    const lostGamesMoney = lostGames?.reduce(
      (acc, game) => acc + game.money,
      0,
    );

    const winGames = lastGameBets?.filter((game) => game.status === 1);
