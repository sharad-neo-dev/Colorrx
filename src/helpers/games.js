...
export function generatePeriod30Sec(gameRepresentationId = 30) {
  const TodayPeriod = moment().format("YYYYMMDD");

  const minuteIndex = getNthMinuteSinceDayStart() + 1;

  const currentSecond = moment().seconds(); // 0–59
  const secondSlot = currentSecond < 30 ? 1 : 2;

  // har minute me 2 round
  const roundIndex = (minuteIndex - 1) * 2 + secondSlot;

  const paddedRoundIndex = _.padStart(
    roundIndex.toString(),
    4,
    "0"
  );
  ...
  const NewGamePeriod =
    `${TodayPeriod}${gameRepresentationId}${paddedRoundIndex}`;
