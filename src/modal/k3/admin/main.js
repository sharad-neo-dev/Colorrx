...
function messNewJoin(data) {
    ...
    if (typeGame == 'two-same') {
        result += `
            ...
            <img class="direct-chat-img" src="/images/myimg.png" alt="message user image">
            <div class="direct-chat-text" style="background-color: #1eb93d"> Join 2 same numbers(${data.listJoin}) with the A mount of money ${Number(data.money) * Number(data.xvalue)}</div>
        `; 
    }
    if (typeGame == 'unlike') {
        result += `
